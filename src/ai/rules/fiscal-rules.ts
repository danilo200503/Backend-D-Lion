  ...
  documento.itens.forEach((item) => {
    ...
    if (operacaoInterna && CFOP_INTERESTADUAL.has(primeiroDigito)) {
      erros.push({
        ...
        explicacao:
          'Foi identificado que o CFOP utilizado normalmente representa operações interestaduais. Entretanto, o emitente e o destinatário estão no mesmo estado, ou seja, a operação parece ser interna. Verifique o enquadramento fiscal da nota.',
    ...
    if (!operacaoInterna && CFOP_INTERNO.has(primeiroDigito)) {
      erros.push({
        ...
        descricao: `CFOP ${item.cfop} incompatível com o tipo da operação${item.xProd ? ` (${item.xProd})` : ''}.`,
        explicacao:
          'Foi identificado que o CFOP utilizado normalmente representa operações internas (dentro do mesmo estado). Entretanto, o emitente e o destinatário estão em estados diferentes, ou seja, a operação parece ser interestadual. Verifique o enquadramento fiscal da nota.',
  ...
  documento.itens.forEach((item) => {
    if (item.cstIcms && !CST_ICMS_VALIDOS.has(item.cstIcms)) {
      erros.push({
        ...
        explicacao:
          'O Código de Situação Tributária informado para o ICMS não corresponde a nenhum dos códigos oficiais previstos na tabela do Convênio ICMS 61/07 (00, 10, 20, 30, 40, 41, 50, 51, 60, 70, 90).',
...
function regraNcmVazio(documento: DocumentoParaAnalise): ErroFiscal[] {
  ...
  return documento.itens
    ...
    .map((item): ErroFiscal => ({
      ...
      explicacao:
        'O NCM (Nomenclatura Comum do Mercosul) é obrigatório em toda NFe e é usado para classificar fiscalmente a mercadoria, definindo alíquotas de tributos federais.',
...
function regraCamposObrigatoriosAusentes(documento: DocumentoParaAnalise): ErroFiscal[] {
  const erros: ErroFiscal[] = [];

  const camposObrigatorios: Array<{ valor: unknown; nome: string; explicacao: string }> = [
...
export const TODAS_AS_REGRAS: RegraFiscal[] = [
